...
class DeformableTransformer(nn.Module):
    ...
    # qt -random_refpoints_xy, +q_splits, +base_size
    def __init__(self, d_model=256, nhead=8, 
        ...
        if q_splits is not None:
            tgt_mask = torch.full([sum(q_splits), sum(q_splits)], fill_value=True)
            ptr = 0
            for n_query in q_splits:
                tgt_mask[ptr: ptr + n_query, ptr: ptr + n_query] = False
                ptr += n_query
        ...
        # qt +
        if two_stage_type == 'no':
            self.init_ref_points(num_queries, q_splits, base_size)  # init self.refpoint_embed
    ...
    def init_ref_points(self, use_num_queries, q_splits=None, base_size=None):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            ...
            if refpoint_embed is not None:
                # qt refpoint_embed -> refpoint_embed.sigmoid()
                refpoint_embed=torch.cat([refpoint_embed.sigmoid(),refpoint_embed_],dim=1)
                tgt=torch.cat([tgt,tgt_],dim=1)
            else:
                refpoint_embed,tgt=refpoint_embed_,tgt_
            ...
            # qt +
            init_box_proposal = refpoint_embed_
        ...
        #########################################################
        # qt +
        if attn_mask is not None and self.tgt_mask is not None:  # for training
            attn_mask[-self.num_queries:, -self.num_queries:] = self.tgt_mask
        elif attn_mask is None:  # for evaluation
            attn_mask = self.tgt_mask

        # qt refpoints_unsigmoid -> reference_points
        ...
        if self.two_stage_type == "no":
            return hs, references, hs_enc, ref_enc, init_box_proposal, self.refpoint_embed.weight
        else:
