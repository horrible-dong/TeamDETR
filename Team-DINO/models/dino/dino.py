...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        # qt
        tr_out = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask)
        if self.two_stage_type == "no":
            hs, reference, hs_enc, ref_enc, init_box_proposal, embedweight = tr_out
        else:
            hs, reference, hs_enc, ref_enc, init_box_proposal = tr_out
        ...
        if self.aux_loss:
            # qt +embedweight
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord_list, embedweight)
    ...
    # qt +refpoint_embed
    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_coord, refpoint_embed=None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if refpoint_embed is None:
...
class SetCriterion(nn.Module):
    ...
    def loss_position(self, outputs, targets, indices, num_boxes):
        ...
        """
        if 'refpoint_embed' not in outputs:
            losses = {"loss_pos": torch.tensor(0., device=outputs['pred_boxes'].device)}
            return losses

        assert 'pred_boxes' in outputs and 'refpoint_embed' in outputs, outputs.keys()
