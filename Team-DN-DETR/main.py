...
def get_args_parser():
    ...
    # about dn args
    parser.add_argument('--use_dn', default=True,
    ...
    parser.add_argument('--modelname', '-m', type=str, default='dn_dab_detr',
                        choices=['dn_dab_detr', 'dn_dab_deformable_detr', 'dn_dab_deformable_detr_deformable_encoder_only'])
    ...
    # qt +
    parser.add_argument('--matcher', type=str, choices=['ori', 'team'])
    parser.add_argument('--q_splits', type=int, nargs='*')  # --q_splits 65 20 15
    ...
    parser.add_argument('--pos_loss_coef', default=5, type=float,
                        help="loss coefficient for position loss")
    ...
    # Traing utils
    parser.add_argument('--output_dir', default='./runs/__tmp__', help='path where to save, empty for no saving')
    ...
    parser.add_argument('--no_ref_update', action='store_true', help="update refpoints per epoch")
    parser.add_argument('--ref_topk', default=300, type=int, help="The number of predictions selected for calculating the centroid")
    parser.add_argument('--ref_topk_fine_tune', default=100, type=int, help="ref_topk in fine-tune stage")
    ...
    # qt +
    # others
    parser.add_argument('--base_size', default=640, type=int, help="Base size for relative scale")
    parser.add_argument('--pos_thr', default=0.25, type=float, help="Threshold in position loss")
...
def main(args):
    ...
    logger = setup_logger(output=None, distributed_rank=args.rank, color=False, name="DAB-DETR")
    ...
    if args.resume:
        ...
        # qt +
        if not args.eval:  # TODO: update refpoints
    ...
    # qt +
    print(model_without_ddp.refpoint_embed.weight.data[:, :4])
    if (args.start_epoch > args.lr_drop or args.drop_lr_now) and not args.no_ref_update:
        print(f"ref_topk is now changed from {args.ref_topk} to {args.ref_topk_fine_tune}")
        args.ref_topk = args.ref_topk_fine_tune
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # qt +
        if epoch == args.lr_drop and not args.no_ref_update:
            print(f"ref_topk is now changed from {args.ref_topk} to {args.ref_topk_fine_tune}")
            args.ref_topk = args.ref_topk_fine_tune
