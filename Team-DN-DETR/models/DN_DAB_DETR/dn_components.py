...
def prepare_for_dn(dn_args, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    if num_patterns == 0:
        num_patterns = 1
    indicator0 = torch.zeros([num_queries * num_patterns, 1]).cuda()  # [Q, 1]
    tgt = label_enc(torch.tensor(num_classes).cuda()).repeat(num_queries * num_patterns, 1)  # [Q, 255]
    tgt = torch.cat([tgt, indicator0], dim=1)   # [Q, 256]
    ...
    if training:
        ...
        labels = torch.cat([t['labels'] for t in targets])  # [num_targets, ]
        boxes = torch.cat([t['boxes'] for t in targets])  # [num_targets, 4]
        ...
        attn_mask = torch.ones(tgt_size, tgt_size).to('cuda') < 0  # [pad_size + Q, pad_size + Q]
    ...
    input_query_label = input_query_label.transpose(0, 1)  # [pad_size + Q, B, 256]
    input_query_bbox = input_query_bbox.transpose(0, 1)  # [pad_size + Q, B, 4]
