...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, caption=None, dpi=140, savedir=None, show_in_console=True, mode=0o777):
        """
        img: tensor(3, H, W)
        ...
        if savedir is not None:
            if caption is None:
                savename = f"{savedir}/{int(tgt['image_id'])}.png"
            else:
                savename = f"{savedir}/{int(tgt['image_id'])}-{caption}.png"
            plt.savefig(savename)
            os.chmod(savename, mode)
        plt.close()
    ...
    def addtgt(self, tgt):
        ...
        # qt +
        # if tgt.get("ref") is not None:
        #     for box in tgt["ref"][-10:].cpu():
        #         unnormbbox = box * torch.Tensor([W, H, W, H])
        #         unnormbbox[:2] -= unnormbbox[2:] / 2
        #         [bbox_x, bbox_y, bbox_w, bbox_h] = unnormbbox.tolist()
        #         boxes.append([bbox_x, bbox_y, bbox_w, bbox_h])
